...
| Beads and Dolt | **Supported** — `GasTownHall.Beads` in the export-only Winget inventory plus a separate manual Dolt release install; use `assets/beads-sync.ps1`. | **Supported** — Beads from `configs/mise_wsl2.toml` and the pinned Dolt release from the WSL Ansible playbook; use `assets/beads-sync.sh`. | **Supported** — separate `beads` and `dolt` Homebrew formulae; use `assets/beads-sync.sh`. | **Supported** — `@beads/bd` from the npm manifest and a separate Dolt release installed by `postCreate.sh`; use `assets/beads-sync.sh` for this dotfiles repository. |
