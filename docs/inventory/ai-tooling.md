...
| OpenCode | **Supported** — Chocolatey `opencode`; export-only inventory, manual clean-host install, always-run v1 hold hook. | **Supported** — exact v1 `opencode` in `configs/mise_wsl2.toml`, installed by WSL Ansible. | **Supported** — `anomalyco/tap/opencode` in `brewfile.txt`, always-run v1 hold hook. | **Supported** — exact v1 `opencode-ai` in `configs/npm_packages.txt`, installed by `postCreate.sh`. See [reviewed v1 updates](../automation/opencode-v1.md). |
...
| codebase-memory-mcp | **Supported** — `.chezmoidata.yaml` owns the version; the checksum-verifying `install_codebase-memory-mcp.ps1` chezmoi hook installs the native binary and the Claude MCP hook registers it. | **Supported** — `.chezmoidata.yaml` owns the version; the WSL Ansible playbook writes its exact mise fragment before installation. | **Supported** — `.chezmoidata.yaml` owns the version; `private_dot_config/mise/conf.d/96-codebase-memory-mcp.toml.tmpl` renders the exact mise pin. | **Supported** — `.chezmoidata.yaml` owns the version rendered as `CBM_VERSION`; `postCreate.sh` installs the checksum-verified portable release and lifecycle wiring registers the Claude MCP server. |
...
- Shared codebase-memory-mcp pin: [`.chezmoidata.yaml`](../../.chezmoidata.yaml) and the [macOS mise fragment](../../private_dot_config/mise/conf.d/96-codebase-memory-mcp.toml.tmpl)
...
- Devcontainer package manifests: [`npm_packages.txt`](../../private_Documents/development/container-dotfiles/devcontainers/gitlab.com/servers-homelab/homelab-IaC/configs/npm_packages.txt), [`promptfoo-runtime/`](../../private_Documents/development/container-dotfiles/devcontainers/gitlab.com/servers-homelab/homelab-IaC/configs/promptfoo-runtime), [`uv_tools.txt`](../../private_Documents/development/container-dotfiles/devcontainers/gitlab.com/servers-homelab/homelab-IaC/configs/uv_tools.txt), and [`pipx_packages.txt`](../../private_Documents/development/container-dotfiles/devcontainers/gitlab.com/servers-homelab/homelab-IaC/configs/pipx_packages.txt)
