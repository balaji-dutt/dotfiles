...
def format_export_markdown(profile: StyleProfile) -> str:
    """Return a pasteable numeric voice profile for external runtimes.

    ChatGPT Skills and similar hosted systems cannot read the local
    `style-memory.json` file. This export deliberately contains only the
    measured numeric targets from `StyleProfile`, never prose samples or
    preference strings, so it can be pasted or uploaded as a portable voice
    anchor with less privacy risk than raw writing samples.
    """
    data = profile.to_dict()
    fields = [
        ("total_words", "sample words"),
        ("sentences", "sentences"),
        ("sentence_length_mean", "sentence length mean"),
        ("sentence_length_stdev", "sentence length stdev"),
        ("sentence_length_cv", "sentence length coefficient of variation"),
        ("word_length_stdev", "word length stdev"),
        ("fragment_rate", "sentence fragment fraction"),
        ("contraction_rate", "contractions per 1k words"),
        ("em_dash_rate", "em dashes per 1k words"),
        ("semicolon_rate", "semicolons per 1k words"),
        ("colon_rate", "colons per 1k words"),
        ("parenthetical_rate", "parentheticals per 1k words"),
        ("type_token_ratio", "type-token ratio"),
        ("function_word_rate", "function words per 1k words"),
        ("avg_commas_per_sentence", "average commas per sentence"),
        ("latinate_ratio", "latinate word fraction"),
        ("first_person_rate", "first-person pronouns per 1k words"),
        ("second_person_rate", "second-person pronouns per 1k words"),
        ("passive_voice_approx", "passive voice approximation per 1k words"),
        ("starts_with_and_but", "sentences starting with And/But fraction"),
    ]
    lines = [
        "# Unslop Voice Profile",
        "",
        "Numeric style targets only. No prose samples are stored here.",
        "This is designed to be safe to paste or upload compared with raw writing, but it is still personal stylometric metadata.",
        "",
        "Use in voice-match mode: match these numeric targets before surface phrasing. Do not invent biography, preferences, facts, or opinions from this profile.",
        "",
        "## Targets",
        "",
    ]
    for key, label in fields:
        lines.append(f"- `{key}` ({label}): `{data[key]}`")
    lines.extend(
        [
            "",
            ...
            "",
            "```json",
            json.dumps({"unslop_voice_profile_version": 1, "profile": data}, indent=2),
            "```",
            "",
        ]
    )
