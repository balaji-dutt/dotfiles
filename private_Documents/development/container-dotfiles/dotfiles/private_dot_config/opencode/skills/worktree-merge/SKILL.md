...
- Pass `--close-beads <issue-id>` only when inspect reports a matching
  OpenCode Beads state for that exact issue.
...
```bash
./assets/agent-wt-merge ff --actor opencode [--update-main] [--close-beads <issue-id>]
```
...
```bash
./assets/agent-wt-merge no-ff --actor opencode -m "<subject>" -m "<body>" [--update-main] [--close-beads <issue-id>]
```
