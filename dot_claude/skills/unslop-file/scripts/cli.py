...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="FORMAT",
        help=(
            "Export the persisted style memory as numeric-only markdown or JSON. "
            "One-shot; exits after. Use markdown for ChatGPT Skills/Library."
        ),
    )
    parser.add_argument(
        "--detector-feedback",
...
def _handle_voice_memory_commands(args: argparse.Namespace) -> int | None:
    """Run voice-memory one-shot commands.

    Returns an exit code when a memory command ran, or None when no command
    applied.
    """
    ...
    if args.export_voice_profile is not None:
        from .style_memory import StyleMemoryError, format_export_markdown, load_profile
        ...
        if profile is None:
            sys.stderr.write(
                "Error: no style memory on file. Run "
                "`unslop --save-voice-profile path/to/sample.md` first.\n"
            )
            return 1
        if args.export_voice_profile == "json":
            payload = {
                "unslop_voice_profile_version": 1,
                "profile": profile.to_dict(),
            }
            sys.stdout.write(json.dumps(payload, indent=2) + "\n")
        else:
            sys.stdout.write(format_export_markdown(profile))
        return 0
