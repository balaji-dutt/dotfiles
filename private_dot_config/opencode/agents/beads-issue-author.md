...
1. Preflight:
   ...
   - Source `$HOME/.local/share/beads-helpers.bash` when readable.
   - Confirm the repo has Beads metadata. For a bare ID, use
     `.beads/metadata.json` `dolt_database` as the prefix when present.
2. Resolve the plan source:
   - Use passed plan text directly, or read only the confirmed file path.
